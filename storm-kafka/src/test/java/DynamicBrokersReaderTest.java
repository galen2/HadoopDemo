...
public class DynamicBrokersReaderTest {
    ...
    private CuratorFramework zookeeper;//操作zookeeper的客户端封装对象
    private TestingServer server;//zookeeper模拟zk服务引擎
    ...
    /**
     * 往zookpeeer节点里面写入数据
     * @param path /brokers/topics/testing/partitions
     * @param data
     * @throws Exception
     */
    ...
    private void writeLeaderDetails(int leaderId, String host, int port) throws Exception {
        String path = dynamicBrokersReader.brokerPath() + "/" + leaderId;///brokers/ids/0
    ...
    /*
     * 测试 创建多个partition对象，在不同给的HOST上面
     */
    @Test
